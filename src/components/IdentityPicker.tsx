          ...
          return (
            <button
              ...
              className={`rounded-xl border px-2 py-2.5 text-sm font-semibold transition
                ${
                  value === name
                    ? 'border-ember bg-ember/15 text-chalk'
                    : 'border-edge/70 bg-ash/50 text-mute'
                }
                ${taken ? 'cursor-not-allowed opacity-25 line-through' : 'active:scale-95'}`}
            >
              {name}
            </button>
          );
        ...
        {AVATARS.map((a) => {
          ...
          return (
            <button
              ...
              aria-label={a.label}
              className={`relative flex aspect-square items-center justify-center rounded-2xl border transition active:scale-95
                ${
                  value === a.key
                    ? 'border-ember bg-ember/10'
                    : 'border-edge/70 bg-ash/50'
                } ${used && value !== a.key ? 'opacity-45' : ''}`}
            >
              <Avatar avatarKey={a.key} size={34} />
              {used && (
